...
def choice_level():
    ...
    bttn( choice_level,850, 400, 'images/level1_btn.png', 'images/level1_hover_btn.png', lambda: switch_window(choice_level, lambda: level_window(GameData[0],'Gamelevel1.db')))
    bttn( choice_level,850, 500, 'images/level2_btn.png', 'images/level2_hover_btn.png', lambda: switch_window(choice_level, lambda: level_window(GameData[2],'Gamelevel2.db')))
    bttn( choice_level,850, 600, 'images/level3_btn.png', 'images/level3_hover_btn.png', lambda: switch_window(choice_level, lambda: level_window(GameData[3],'Gamelevel3.db')))
...
class Show_data:
    ...
    def ensure_table_exists(self):
        ...
        conn.close()
class Win_window:
    def __init__(self, game_time,db_name):
        self.master = Toplevel()  # Use the passed master instance
        self.game_time = game_time
        self.db_name = db_name  # Store the passed game_time as an instance variable
        self.master.title("Win_window")
        self.master.geometry("320x320")

        bg = Image.open('images/image.png')
        ...
        self.f_name.place(x=120, y=150)

        # Label for name
        self.f_name_label = Label(self.master, text="Your Name",bg="#493294",font=("Helvetica", 12,'bold'),fg="#fff")
        self.f_name_label.place(x=20, y=150)

        # Submit button
        bttn(self.master,115, 250,'images/save_btn.png','images/save_hover_btn.png',self.submit)
        # self.add_btn = Button(self.master, text='Submit', command=self.submit)
        # self.add_btn.place(x=130, y=250)
...
class Stopwatch(Label):
    def __init__(self, master=None, **kwargs):
        ...
        self._running = False
        self.config(text="00:00:00", font=('Arial', 20,'bold'),bg="#fff")
...
class CrosswordGame:
    def __init__(self, canvas, crossword_data, level_win,db_name):
        self.canvas = canvas
        ...
        self.stopwatch.place(x=900, y=130)  # Adjust the position as needed
    ...
    def submit_button(self, event=None):
        ...
        if all_correct:
            ...
            Win_window(formatted_play_time,self.db_name)

            # if answer == 'yes':
            #     level_num += 1
            #     if level_num < len(Show_data):
            #         switch_window(self.level_win, lambda: level_window(Show_data[level_num]))
            #     else:
...
def level_window(crossword_data,db_name):
    level_win = Tk()
...
def rank_gui():
    ...
    frame.pack(padx=660, pady=400, fill='both', expand=True)

    # Instantiate Show_data with the first level by default
    Show_data(frame, 'Gamelevel1.db')

    def update_Show_data(db_name):
        # Update the Show_data instance with the selected level
        Show_data(frame, db_name)

    # Example buttons
    bttn(root, 659, 340, 'images/1level.png', 'images/1hover_level.png', lambda: update_Show_data('Gamelevel1.db'))
    bttn(root, 860, 340, 'images/2level.png', 'images/2hover_level.png', lambda: update_Show_data('Gamelevel2.db'))
    bttn(root, 1061, 340, 'images/3level.png', 'images/3hover_level.png', lambda: update_Show_data('Gamelevel3.db'))
    bttn(root, 100, 900, 'images/back_btn.png', 'images/back_hover_btn.png', lambda: switch_window(root,start_welcome_window))

    root.mainloop()

# Test function call (replace Show_data with your actual crossword data)
# level_window(Show_data[0])
start_welcome_window()
